...
│   │   ├── test_views.py           # Auth, CRUD, formset, CSV, ownership
│   │   └── test_volume.py          # Volume/stress tests at 4 data scales
│   └── templates/
│       └── dashboard/
│           ├── _navbar.html        # Shared navigation bar (Svelte island mount)
│           ├── dashboard.html      # Main dashboard page (mockup)
│           ├── login.html          # Login page (Svelte island form)
│           ├── register.html       # Registration page (Svelte island form)
│           ├── templates_list.html # Template listing with search
│           ├── template_create.html# New template creation (Svelte island form)
│           ├── template_detail.html# Template detail + inline edit (Svelte island)
...
│       └── components/
│           ├── Dashboard.svelte    # Dashboard widget
│           ├── Navbar.svelte       # Navigation bar component
│           ├── LoginForm.svelte    # Login form island
│           ├── RegisterForm.svelte # Registration form island
│           ├── TemplateForm.svelte # Template create/edit form island
│           └── ui/                 # Reusable UI components
│               ├── Input.svelte    # Text input with validation styling
│               ├── Select.svelte   # Dropdown select with custom arrow
│               └── Textarea.svelte # Textarea with validation styling
│
...
|-----------|-------------|----------|
| `Navbar.svelte` | `#svelte-navbar` | Responsive navigation bar with user info and logout |
| `Dashboard.svelte` | `#svelte-dashboard` | Dashboard widget (mockup) |
| `LoginForm.svelte` | `#svelte-login-form` | Login form with error display |
| `RegisterForm.svelte` | `#svelte-register-form` | Registration form with validation |
| `TemplateForm.svelte` | `#svelte-template-form` | Template create/edit form with dynamic column editor |

**Reusable UI Components (`frontend/src/components/ui/`):**

| Component | Props | Purpose |
|-----------|-------|---------|
| `Input.svelte` | `name`, `type`, `value`, `error`, `label`, etc. | Text input with focus effects and error styling |
| `Select.svelte` | `name`, `value`, `options`, `error`, `label` | Dropdown select with custom arrow icon |
| `Textarea.svelte` | `name`, `value`, `error`, `label` | Multi-line text input with error styling |

**Data Bridge (Django -> Svelte):**
Complex data like formset state (CSRF tokens, initial column values, data type choices) is passed from Django to Svelte via a `window.__TEMPLATE_DATA__` JavaScript object rendered inside a `<script>` tag in the Django template. The Svelte component reads this data during one-time initialization.
...
The test suite lives in `dashboard/tests/` with **73 total tests** across 3 modules:

```
dashboard/tests/
├── __init__.py        # Re-exports all test classes
├── test_models.py     # 10 tests — Model CRUD, constraints, cascades
├── test_views.py      # 35 tests — Auth, CRUD, formset, CSV, ownership
...
| Test Class                  | Tests | Covers                                                     |
|-----------------------------|-------|------------------------------------------------------------|
| `AuthViewTests`             | 9     | Login/register/logout pages, success/failure, redirects    |
| `DashboardViewTests`        | 2     | Login requirement, authenticated access                    |
| `TemplateListViewTests`     | 6     | Login redirect, search, ownership isolation, response time |
| `TemplateCreateViewTests`   | 6     | Form submission (Svelte format), duplicate names, all data types |
| `TemplateDetailViewTests`   | 6     | Inline edit, add/delete columns, ownership, formset POST   |
| `TemplateDeleteViewTests`   | 3     | Confirmation page, deletion, ownership isolation           |
| `TemplateDownloadCSVTests`  | 3     | CSV download, content validation, ownership isolation      |
...
# Run only view tests (auth, CRUD, formset, CSV)
...
### Svelte Form Integration

Template create and edit forms use the `TemplateForm.svelte` component which:
- Renders a dynamic column editor with add/remove functionality
- Submits data as a standard Django `modelformset_factory` POST format (prefix `form-`)
- Handles CSRF tokens, formset management fields (`TOTAL_FORMS`, `INITIAL_FORMS`), and per-column hidden fields (`id`, `order`, `DELETE`)
- Receives initial data from Django via `window.__TEMPLATE_DATA__` JavaScript bridge
